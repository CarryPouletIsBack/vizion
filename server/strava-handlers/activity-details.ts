...
export async function activityDetailsHandler(req: VercelRequest, res: VercelResponse) {
  if (req.method !== 'GET') return res.status(405).json({ error: 'Method Not Allowed' })
  const authHeader = req.headers.authorization
  const accessToken = authHeader?.replace('Bearer ', '')
  if (!accessToken) return res.status(401).json({ error: 'Missing access token' })
  const activityId = req.query.activity_id as string
  if (!activityId) return res.status(400).json({ error: 'Missing activity_id parameter' })
  try {
    const activityResponse = await fetch(`https://www.strava.com/api/v3/activities/${activityId}`, {
      headers: { Authorization: `Bearer ${accessToken}` },
    })
    if (!activityResponse.ok) {
      if (activityResponse.status === 401) return res.status(401).json({ error: 'Token expired or invalid' })
      if (activityResponse.status === 404) return res.status(404).json({ error: 'Activity not found' })
      return res.status(activityResponse.status).json({ error: await activityResponse.text() })
    }
    ...
    try {
      ...
      if (streamsResponse.ok) streams = await streamsResponse.json()
    } catch {
      // ignore
    }
    ...
    if (activity.splits_metric && Array.isArray(activity.splits_metric) && activity.splits_metric.length > 0) {
      splitsPerKm = activity.splits_metric.map((split: any, index: number) => ({
        ...
        paceMinPerKm: split.moving_time && split.distance ? (split.moving_time / 60) / (split.distance / 1000) : undefined,
      }))
      ...
      for (let km = 1; km <= Math.floor(totalKm); km++) {
        ...
        splitsPerKm.push({ km, movingTimeSec: elapsedSec, elapsedTimeSec: elapsedSec, paceMinPerKm: pace })
      }
    }
